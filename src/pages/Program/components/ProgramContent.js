	...
	const saveTitleChange = async (card, newTitle) => {
		if (newTitle !== card.title) {
			try {
				...
				setCards((prevCards) => prevCards.map((c) => (c.id === card.id ? { ...c, title: newTitle } : c)));
					...
					<div key={index} className="program-content__card" onClick={() => handleCardClick(card.id)}>
						<div className="program-content__card-header">
							{editingCardId === card.id ? (
								<input
									className="program-content__card-title-button"
									type="text"
									value={editedTitle}
							...
							<button
								...
								className="program-content__icon-button"
								onClick={(event) => {
						...
						<p className="program-content__card-days">{card.workoutDaysCount > 0 ? `${card.workoutDaysCount} training day${card.workoutDaysCount > 1 ? 's' : ''}` : 'No training days'}</p>
