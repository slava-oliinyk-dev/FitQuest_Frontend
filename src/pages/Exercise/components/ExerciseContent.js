                            ...
                            <div className='exercise-content__card-title-button'>
                                {
                                    editingExerciseId === exercise.id ?
                                        <input className='exercise-content__edit-title-input' type="text"
                                        ...
                                        />
                                        :
                                        <span className='exercise-content__card-title' lang="en" >{exercise.name}</span>
                                ...
                                <div>
                                    ...
                                    {isNotesModalVisible && activeNotesId === exercise.id && (
                                        ...
                                        >
                                            ...
                                            <textarea className='exercise-content__notes-modal-textarea' placeholder='Add notes, for example, about exercise technique or any tips to improve performance...' onBlur={() => handleNotesBlur(exercise, currentNotesValue)} onChange={handleNotesValueChange}>{exercise.note}</textarea>
                                            <div className='exercise-content__notes-modal-buttons'>
                                                <button className='exercise-content__notes-modal-button-cancel' onClick={closeNotesModal}>Cancel</button>
                                                <button className='exercise-content__notes-modal-button-save' onClick={closeNotesModal}>Save</button>
                                            </div>
                                        </NotesModal>
                                    )}
                                    <button className="exercise-content__card-button" ref={(el) => {
                                        ...
                                        <SettingsModal isVisible={true} onClose={closeModal} targetRef={buttonRefs.current[exercise.id]} offsetX={-145}
                                            ...
                                            <div className="exercise-content__settings-modal">
                                                <ul className="exercise-content__settings-menu">
                                                    <li onClick={(e) => { e.stopPropagation(); handleChangeTitleClick(exercise) }}><img className="exercise-content__settings-icon" src={editIcon} alt="" /> Change name</li>
                                                    <li onClick={(e) => { e.stopPropagation(); handleDeleteExercise() }}><img className="exercise-content__settings-icon" src={deleteIcon} alt="" /> Delete</li>
                                                </ul>
                                            </div>
                                        </SettingsModal>
                                    )}
                            ...
                            <div className='exercise-content__inputs'>
                                <div className='exercise-content__inputs-input-label'>
                                    {editingParameters?.id === exercise.id && editingParameters?.field === 'sets' ? (
                                        <input className='exercise-content__inputs-input' type="number" placeholder='0' value={currentParametersValue}
                                        ...
                                        <div className='exercise-content__inputs-div' onClick={() => handleParametersClick(exercise, 'sets')}>{exercise.sets}</div>
                                    )}
                                ...
                                <div className='exercise-content__inputs-input-label'>
                                    {editingParameters?.id === exercise.id && editingParameters?.field === 'weight' ? (
                                        ...
                                        <div className='exercise-content__inputs-div' onClick={() => handleParametersClick(exercise, 'weight')}>{exercise.weight}</div>
                                    )}
                                ...
                                <div className='exercise-content__inputs-input-label'>
                                    {editingParameters?.id === exercise.id && editingParameters?.field === 'repetitions' ? (
                                        ...
                                        <div className='exercise-content__inputs-div' onClick={() => handleParametersClick(exercise, 'repetitions')}>{exercise.repetitions}</div>
                                    )}
